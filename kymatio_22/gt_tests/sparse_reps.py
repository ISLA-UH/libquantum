...
def gt_blast(sig_frequency_hz:float = 1.,
             window_cycles: float = 64,
             bit_loss: float = 6,
             sample_rate_hz: float =None):

    # Pulse period
    pseudo_period_s = 1/sig_frequency_hz
    # GT pulse relative to target (main) period
    window_duration_s = window_cycles*pseudo_period_s
    # This will be the time to use 2^n. Note target frequency is used!
    if sample_rate_hz is None:
        sample_rate_hz = 32*sig_frequency_hz
    time_points = int(window_duration_s*sample_rate_hz)
    ...
    return sig_theory, sig, time_s, time_shifted_s, sig_frequency_hz
...
if __name__ == "__main__":
    ...
    sig_gt, sig_n, time_s, time_shifted_s, frequency_sig_hz = \
        gt_blast(sig_frequency_hz=frequency_main_hz,
                 window_cycles=64, bit_loss=6,
